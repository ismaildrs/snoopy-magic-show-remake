...
void moveBall(){
    Pos move1 = (Pos){.i = (game.ball.i +(cell_size*0.45)/2)/cell_size, .j = (game.ball.j + (cell_size*0.45) +16)/cell_size};
    Pos move2 = (Pos){.i = (game.ball.i +(cell_size*0.45)/2)/cell_size, .j = (game.ball.j -16)/cell_size};
    Pos move3 = (Pos){.i = (game.ball.i +cell_size*0.45+16)/cell_size, .j = (game.ball.j+(cell_size*0.45)/2)/cell_size};
    Pos move4 = (Pos){.i = (game.ball.i-16)/cell_size, .j = (game.ball.j + (cell_size*0.45)/2)/cell_size};
    if(game.ball_dir_y == Down){
        if(game.ball.j<screen_size-cell_size*0.45 && validMove(move1)) game.ball.j += 30;
        else game.ball_dir_y = Up;
    }
    else if(game.ball_dir_y == Up ){
        if(game.ball.j>0&&validMove(move2)) game.ball.j -= 30;
        else game.ball_dir_y = Down;
    }
    if(game.ball_dir_x == Right){
        if(game.ball.i<screen_size-cell_size*0.45 && validMove(move3)) game.ball.i+= 30;
        else game.ball_dir_x = Left;
    }
    else if(game.ball_dir_x == Left){
        if(game.ball.i>0 && validMove(move4)) game.ball.i-= 30;
...
int snoopyAndBall(){
    if(game.snoopy.i == (game.ball.i+(cell_size*0.45)/2)/cell_size && game.snoopy.j == (game.ball.i+(cell_size*0.45)/2)/cell_size)
        return 1;
    return 0;
}
...
void snoopyAndBird(){
    ...
    for(int i = 0; i<2; i++){
        for(int j=0; j<2; j++){
            if(bird[i][j].i == game.snoopy.i && bird[i][j].j == game.snoopy.j) game.score++;
        }
    }
}

int gameState(){
    if(!snoopyAndBall() && game.score <= 4) return 1;
    return 0;
}